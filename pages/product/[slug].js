      ...
      <div className="container px-5 py-16 mx-auto">
        <div className="lg:w-4/5 mx-auto flex flex-wrap">
          ...
          <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
            <h2 className="text-sm title-font text-gray-500 tracking-widest">NexCritique.com</h2>
            ...
            <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
              <div className="flex">
                ...
                {Object.keys(variants).includes('white') && Object.keys(variants['white']).includes(size) && <button onClick={() => refreshVariant('white', size)} className={`border-2 ml-1 bg-white rounded-full w-6 h-6 focus:outline-none ${color === 'white' ? 'border-black' : 'border-gray-300'}`}></button>}
                ...
                {Object.keys(variants).includes('purple') && Object.keys(variants['purple']).includes(size) && <button onClick={() => refreshVariant('purple', size)} className={`border-2 ml-1 bg-purple-500 rounded-full w-6 h-6 focus:outline-none ${color === 'purple' ? 'border-black' : 'border-gray-300'}`}></button>}
                {Object.keys(variants).includes('pink') && Object.keys(variants['pink']).includes(size) && <button onClick={() => refreshVariant('pink', size)} className={`border-2 ml-1 bg-pink-500 rounded-full w-6 h-6 focus:outline-none ${color === 'pink' ? 'border-black' : 'border-gray-300'}`}></button>}
              </div>
            ...
            <div className="flex">
              ...
              {product.availableQty<= 0 && <span className="title-font font-medium text-2xl text-gray-900">Product went out of stock</span>}
              ...
              <button disabled={product.availableQty<=0} onClick={() => { addToCart(slug, 1, product.price, product.title, size, color) }} className="flex ml-8 text-white bg-pink-500 disabled:bg-pink-300 border-0 py-2 px-2 md:px-6 focus:outline-none hover:bg-pink-600 rounded">Add to cart</button>
            ...
            <div className='pin mt-6 flex space-x-2 text-sm'>
              <input onChange={onChangePin} type="text" className='px-2 border-2 border-pink-500 rounded-md' placeholder='Enter your pincode' />
              <button onClick={checkService} className='text-white bg-pink-500 border-0 py-2 px-6 focus:outline-none hover:bg-pink-600 rounded'>
                Check
              </button>
            </div>
            {!service && <div className='text-red-700 mt-3'> We do not Deliver at this pincode </div>}
...
export async function getServerSideProps(context) {
  ...
  return {
    props: {error, product: JSON.parse(JSON.stringify(product)), variants: JSON.parse(JSON.stringify(colorSizeSlug)) }
  }
}

export default Slug

// 1 title ki multiple tshirts hai jiska color aur size vary kar rha hai aur vahi ham display karva rahe hai change karvake slug.js mein